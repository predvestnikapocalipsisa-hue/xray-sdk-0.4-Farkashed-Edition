...
struct UndoItem
{
	...
	UndoItem() : m_Data(nullptr), m_Size(0) {}
	~UndoItem()
	{
		xr_free(m_Data);
	}

	UndoItem(const UndoItem& other)
	{
		m_Size = other.m_Size;
		if (m_Size > 0)
		{
			m_Data = (u8*)xr_malloc(m_Size);
			CopyMemory(m_Data, other.m_Data, m_Size);
		}
		else
		{
			m_Data = nullptr;
	...
	UndoItem& operator=(const UndoItem& other)
	{
		if (this != &other)
		{
			xr_free(m_Data);
	...
	UndoItem(UndoItem&& other) noexcept
	{
		m_Data = other.m_Data;
		m_Size = other.m_Size;
	...
	UndoItem& operator=(UndoItem&& other) noexcept
	{
		if (this != &other)
		{
			xr_free(m_Data);
...
public:
	...
	bool LoadStream(IReader& F, bool bUndo);
