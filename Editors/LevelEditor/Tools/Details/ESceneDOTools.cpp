...
{
    Msg("! EDetailManager: destructor start"); FlushLog();
    Clear();
    Unload();
    Msg("! EDetailManager: destructor end"); FlushLog();
}
        ...
        if (1 == priority)
        {
            if (false == strictB2F)
            {
                if (m_Flags.is(flSlotBoxesDraw))
                {
                    ...
                    // FIX: увеличены дистанции + добавлен гистерезис дл€ исключени€ мигани€
                    // на границе дальности видимости при движении камеры
                    float dist_lim_in = 70.f * 70.f;   // порог по€влени€ (ближе)
                    float dist_lim_out = 85.f * 85.f;   // порог исчезновени€ (дальше)
                    ...
                    for (u32 z = 0; z < dtH.size_z; z++)
                    {
                        ...
                        for (u32 x = 0; x < dtH.size_x; x++)
                        {
                            ...
                            // FIX: гистерезис Ч слот Ђвключаетс€ї по dist_lim_in,
                            // Ђвыключаетс€ї по dist_lim_out, убирает мигание на границе
                            ...
                            // FIX: используем testSphere с маской дл€ точного frustum culling.
                            // getMask() возвращает маску всех активных плоскостей frustum'а.
                            // testSphere_dirty не трогаем Ч он нестабилен на границе,
...
{
    // FIX: InvalidateCache вызываетс€ только при реальном изменении плотности,
    // не каждый кадр Ч чтобы не сбрасывать кеш видимости посто€нно
    InvalidateCache();
}

void EDetailManager::OnBaseTextureChange(PropValue* prop)
{
    Msg("! EDetailManager::OnBaseTextureChange: start, proxy='%s'", m_BaseTextureProxy.c_str()); FlushLog();
    m_Base.name = m_BaseTextureProxy;
    m_Base.OnImageChange(prop);
    Msg("! EDetailManager::OnBaseTextureChange: InvalidateSlots"); FlushLog();
    InvalidateSlots();
    ELog.DlgMsg(mtInformation, "Texture changed. Reinitialize objects.");
    Msg("! EDetailManager::OnBaseTextureChange: end"); FlushLog();
}

void EDetailManager::OnBaseTextureCuttonClick(ButtonValue* B, bool& bModif, bool& bSuccess)
{
    OPENFILENAME ofn;
    char szFile[260];
    ...
    if (GetOpenFileName(&ofn))
    {
        ...
        const char* textures_root = strstr(ofn.lpstrFile, "textures\\");
        ...
        if (textures_root)
        {
            ...
            m_BaseTextureProxy = relative_path;

            if (!m_Base.LoadImage(relative_path))
            {
                Msg("! [Detail Error] LoadImage failed for: %s", relative_path);
            }
            ...
            UI->RedrawScene();
            OnBaseTextureChange(nullptr);

            Msg("~ [Detail Mod] Success! Path: %s", relative_path);
        }
        else
        {
            Msg("! [Detail Error] ‘айл должен находитьс€ внутри gamedata\\textures\\!");
...
{
    m_BaseTextureProxy = m_Base.name;
    PropValue* P;
