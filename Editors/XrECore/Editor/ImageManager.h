...
public:
    bool MakeGameTexture(ETextureThumbnail* THM, LPCSTR game_name, u32* load_data, u32 w, u32 h);
...
private:
    // NEW: единая точка обязательной классификации типа текстуры по
    // соглашению об именах ("*_bump" -> ttBumpMap, "*_det"/"*_mask" -> ttImage).
    // В отличие от точечных проверок в CreateTextureThumbnail/ETextureThumbnail::Load(),
    // которые срабатывают только при полном отсутствии .thm на диске, эта
    // функция вызывается на каждую (ре)синхронизацию и ПЕРЕЗАПИСЫВАЕТ уже
    // загруженный/дефолтный type - это нужно, чтобы самоисправлять .thm,
    // однажды сохранённые с неверным типом (замкнутый круг Load()->Save()
    // с одним и тем же неверным значением).
    void ApplyNamingConventionType(ETextureThumbnail* THM, LPCSTR base_name);
};
