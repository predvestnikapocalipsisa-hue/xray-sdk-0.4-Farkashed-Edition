...
BOOL TokenOnEdit(PropItem *prop, bool &change)
{
	...
	if (ImGui::Combo(
			"##value", &index, [](void *data, int idx, const char **out_text) -> bool
			{
				...
				temp = XrUIManager::ConvertCP1251ToUTF8(reinterpret_cast<xr_token*>(data)[idx].name);
				*out_text = temp.c_str();
				return true; 
			},
...
BOOL RTokenOnEdit(PropItem *prop, bool &change)
{
	...
	if (ImGui::Combo(
			...
			{
				static xr_string temp;
				temp = XrUIManager::ConvertCP1251ToUTF8(reinterpret_cast<xr_rtoken*>(data)[idx].name.c_str());
				*out_text = temp.c_str();
				return true; 
			},
...
void UIPropertiesItem::DrawProp()
{
	...
	auto DrawTextProperty = [&](const char* text)
	{
		...
		xr_strcat(Str, "...");
		ImGui::Button(DX2U(Str), ImVec2(-1, 0));
	...
	case PROP_CHOOSE:
	{
		...
		if (ImGui::Button(DX2U(text.c_str()), ImVec2(-1, 0)))
		{
			PropItem *prop = node;
...
#pragma TODO("TSMP: РЅРµ РЅР°РґРѕ Р»Рё С‚СѓС‚ С‚РѕР¶Рµ Modified() ?")
	...
	case PROP_SH_TOKEN:
	{
		...
		if (ImGui::Combo("##value", &index, [](void *data, int idx, const char **out_text) -> bool
		{
			static xr_string temp;
			temp = XrUIManager::ConvertCP1251ToUTF8(reinterpret_cast<TokenValueSH::Item*>(data)[idx].str);
			*out_text = temp.c_str();
			return true;
	...
	case PROP_TEXTURE2:
	{
		...
		{
			static xr_string temp;
			temp = XrUIManager::ConvertCP1251ToUTF8(reinterpret_cast<const char**>(data)[idx]);
			*out_text = temp.c_str();
			return true;
		}, reinterpret_cast<void*>(const_cast<LPCSTR*>(List)), cnt))
		{
			if (index == 0)
			{
	...
	case PROP_CLIST:
	{
		...
		if (ImGui::Combo(
				...
				{
					static xr_string temp;
					temp = XrUIManager::ConvertCP1251ToUTF8(reinterpret_cast<xr_string*>(data)[idx].c_str());
					*out_text = temp.c_str();
					return true; 
				},
				reinterpret_cast<void *>(V->items), i))
	...
	case PROP_RLIST:
	{
		...
		if (ImGui::Combo(
				...
				{
					static xr_string temp;
					temp = XrUIManager::ConvertCP1251ToUTF8(reinterpret_cast<shared_str*>(data)[idx].c_str());
					*out_text = temp.c_str();
					return true; 
				},
				reinterpret_cast<void *>(V->items), i))
	...
	case PROP_GAMETYPE:
	{
		...
		R_ASSERT(V);
		ImGui::Text(DX2U(node->GetDrawText().c_str()));
