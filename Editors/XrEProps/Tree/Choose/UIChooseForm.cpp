...
bool UIChooseForm::GetResult(bool &change, shared_str &result)
{
    ...
    if (!Form->bOpen)
    {
        Msg("! UIChooseForm::GetResult: closing, m_Result=%d", Form->m_Result); FlushLog();
        if (Form->m_Result == R_Ok)
        {
            ...
            change = true;
            Msg("! UIChooseForm::GetResult: OK, result='%s'", result.c_str()); FlushLog();
            xr_delete(Form);
            Form = nullptr;
            return true;
        }
        ...
        change = false;
        Msg("! UIChooseForm::GetResult: Cancel"); FlushLog();
        xr_delete(Form);
        Form = nullptr;
        return true;
    }
...
bool UIChooseForm::GetResult(bool& change, xr_string& result)
{
	...
	if (Form->m_Result == R_Ok)
	{
		...
		change = true;
        Msg("! UIChooseForm::GetResult(xr_string): OK, result='%s'", result.c_str()); FlushLog();
		xr_delete(Form);
        Form = nullptr;
		return true;
	}
	...
	change = false;
    Msg("! UIChooseForm::GetResult(xr_string): Cancel"); FlushLog();
	xr_delete(Form);
    Form = nullptr;
	return true;
}
...
bool UIChooseForm::GetResult(bool& change, xr_vector<xr_string>& result)
{
	...
	if (Form->m_Result == R_Ok)
	{
		...
		change = true;
        Msg("! UIChooseForm::GetResult(xr_vector): OK, count=%d", result.size()); FlushLog();
		xr_delete(Form);
        Form = nullptr;
		return true;
	}
	...
	change = false;
    Msg("! UIChooseForm::GetResult(xr_vector): Cancel"); FlushLog();
	xr_delete(Form);
    Form = nullptr;
	return true;
}
